...
function generateProfile( el, profile ) {
	...
	var linklist = generateLinkList( profile.links, profile.display_name );
...
function generateLinkList( links, display_name ) {
	var output = [];
	for ( var label in links ) {
		var href = links[ label ];

		output.push( generateLink( label, href, display_name ) );
	}

	return output.join( "" );
}

function generateLink( label, href, display_name ) {
	var output = tophatProfileLinks
		.replace( /{{label}}/ig, label )
		.replace( /{{href}}/ig, href );

	if(label === display_name)
		output = output.replace(/{{class}}/ig, "class=\"user-name\"");
	else
		output = output.replace(/{{class}}/ig, "");
