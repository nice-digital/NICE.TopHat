...
function getLabel(element) {
	...
	if(element.className.indexOf("menu-profile") > -1)
		menuStateLabel = "Profile" + (element.getAttribute("aria-expanded") == "true" ? " expanded" : " collapsed");
	if(element.id === "menu-evidence")
		menuStateLabel = "Evidence services" + (element.getAttribute("aria-expanded") == "true" ? " expanded" : " collapsed");
	else if(element.className.indexOf("logo") > -1)
		menuStateLabel = "Logo";
	else if(element.className.indexOf("user-name") > -1)
		menuStateLabel = "Edit profile";

	menuStateLabel = menuStateLabel || element.textContent || element.innerText || element.innerHTML;

	menuStateLabel = trimWhiteSpace(menuStateLabel);

	return menuStateLabel;
}

/**
 * Trims whitespace from the start and end of a string
 * @param  {String} str The string from which to remove whitepspace
