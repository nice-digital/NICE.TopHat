...
function keyHandler(e) {
	...
	switch(keyCode) {
		...
		case KeysDict.down:
			...
			break;
		case KeysDict.home:
			domUtils.getLinksAndButtons(menuIdsWithinScope)[0].focus();
			break;
		case KeysDict.end:
			var links = domUtils.getLinksAndButtons(menuIdsWithinScope);
...
function searchHandler( ev ) {
	...
	if ( target.className && ~target.className.indexOf( "nice-search" ) &&
		...
		var term = ( q !== placeholder ) ? encodeURI( q.replace(/\s/g, "+") ) : "";
