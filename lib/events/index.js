...
function attachTophatEvents( document, tophatElement ) {
    ...
    utils.attachDomEvent( document, 'keydown', utils.proxy( tophatElement, keydownHandler ) );
...
function keydownHandler(e){
	...
	while (a) {
	    els.unshift(a);
	    a = a.parentNode;
	    var menus = document.getElementsByClassName('tophat-inner');

	    for(var c = 0; c < menus.length; c++){
	    	var isNotSearchBarDiv = menus[c].parentNode.getAttribute('id') !== "nice-global";
	    	if(menus[c] === a && isNotSearchBarDiv){
				...
				switch(e.keyCode){
			        ...
			        case keysDict.space :
				        e.preventDefault();
		        		if(keyboardNav.openMenu()){
		        			keyboardNav.resetPos();
		        		}
		        		else{
		        			keyboardNav.goToLink();
		        		}
				        break;
