...
function keyHandler(e) {
	...
	function getContainersIdsOfMenuItemsInScope(parentMenuOfCurrentEl){
		if(parentMenuOfCurrentEl){
			parentMenuController = domUtils.getControllingElementId(parentMenuOfCurrentEl);
			return domUtils.getIdsOfControlledElements(parentMenuController) || [parentMenuController];
		}
		return null;
	}

	//Get containers of current displayed menu items
	var parentMenuEl = domUtils.closestWithClass(target, "menu");
	var menuIdsWithinScope = getContainersIdsOfMenuItemsInScope(parentMenuEl);
...
function focusOnFirstMenuItem(elementClass){
	var elControls = document.getElementById(elementClass).getAttribute("aria-controls");
	elControls = elControls.split(" ")[0];
	var menu = document.getElementById(elControls);
