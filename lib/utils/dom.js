...
utils.getPreviousLink = function(containersArray, currentLink) {
	...
	var links = utils.getLinksAndButtons(containersArray),
 ...
 * @return     {Array}  The links or buttons within the container.
 */
utils.getLinksAndButtons = function(containerIds) {
	var links = [];
	for(var x = 0; x < containerIds.length; x++){
		var container = document.getElementById(containerIds[x]);
		var linksInContainerX = utils.toArray(container.querySelectorAll("a, button"));
 ...
 *
 * @param      {HTMLElement}  { The element to remove class from. }
 * @param      {String}  { The class to remove }
 * @return     {String}  { The new classlist }
 */
